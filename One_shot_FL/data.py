...
def get_dataset(datatype, n_client, n_c, alpha, partition_equal=True):
    ...
    trans_svhn = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.4377, 0.4438, 0.4728), (0.1980, 0.2010, 0.1970))])


    if(datatype=='CIFAR10' or datatype=='SVHN' or datatype=='SVHN_original' or datatype == 'GTSRB' or datatype=='CIFAR100' or datatype =='FashionMNIST' or datatype == 'CINIC10'):
        ...
        if(datatype=='SVHN'):
            dataset_train_global = datasets.SVHN('./data/svhn', split="train",download=True, transform=trans_svhn)
            dataset_test_global = datasets.SVHN('./data/svhn', split="test",download = True, transform=trans_svhn)

        if(datatype=='SVHN_original'):
