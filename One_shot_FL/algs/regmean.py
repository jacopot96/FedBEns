...
def regmean_global_merge(p, args, net_glob, models, model_vectors, dataset_train, dataset_val):
    ...
    for i in range(n):
        ...
        model_vector_quantized = quantize_layer(model_vectors[i],model_new,None)
        vector_to_parameters(model_vector_quantized, model_new.parameters())
        new_models_list.append(model_new)

    compress_regmean_gram(grams_list,models[0],False)
    a_range = [0.1, 0.9]
    best_params = None
    best_acc = 0
    for a in a_range:
      ...
      val_test_acc, val_test_loss,_ = test_img(regmean_model, dataset_val, args)
