...
parser.add_argument('--seed', type=int, required=False, default = 32)
...
filename = "one_shot_results_"+str(seed)+"_"+"alpha:"+str(alpha)+"_"+dataset+"_"+model_name+"_"+"_"+str(local_epochs)+str(algs_to_run)
...
for alg in algs_to_run:
  ...
  for t in range(0,args['rounds']):
    ...
    for i in ind:
        ...
        models.append(model)
        test_acc, test_loss,_ = test_img(model, dataset_test_global,args)
        ...
        for layer_id in F_kfac.data.keys():
          ...
          total_param_kfac+=len(F_kfac.data[layer_id][0])**2+len(F_kfac.data[layer_id][1])**2

        print('total_param_kfac',total_param_kfac)
    ...
    test_acc, test_loss,_ = test_img(net_glob, dataset_test_global,args)
