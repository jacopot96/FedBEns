...
def compress_mat(A, s_quant, s_svd,compression=True):
    if compression==True:
        ...
        A_svd = torch.linalg.svd(A)
        A_final = torch.mm(torch.mm(A_svd[0],torch.diag(A_svd[1])),A_svd[2])
        return  A_final


def compress_regmean_gram(gram_matrix_list, model, apply_compression=False):
    ...
    # Compute the total number of parameters in the Gram matrix of each layer
    for layer_id in gram_matrix.keys():
        d1 += torch.numel(gram_matrix[layer_id])
    ...
    if apply_compression:
        ...
        for layer_id in gram_matrix.keys():
            b1 = gram_matrix[layer_id].shape[0]
            d_new += 2 * b1 * min(b1, s_svd) * (s_quant + 1) + min(b1, s_svd) * (s_quant + 1) + 3 * 32

        print("Ratio after compression:", (d_new + 16 * d) / (32 * d))  # Aim for this ratio close to 1
        ...
        print("Compression skipped. Returning Gram matrix without modification.")

    return gram_matrix_list



def compress_fisher_kfac(args, F_kfac_list_org, models, model_vectors, dataset_train, p, apply_compression=15):
    ...
    # Create dummy KFAC matrices for each client
    for i in range(n):
        ...
        # Compute total number of parameters in KFAC matrices
        if i == 0:
            for layer_id in F_kfac.data.keys():
                d1 += torch.numel(F_kfac.data[layer_id][0]) + torch.numel(F_kfac.data[layer_id][1])

        F_kfac_list.append(F_kfac)
   
    ### Compression starts here
    if apply_compression is not None:
        for i in range(n):
            ...
            if(i==0):
                ...
                # Apply compression to KFAC matrices
            for layer_id in F_kfac.data.keys():
        ...
        print("Compression and quantization skipped. Returning unaltered KFAC matrices.")
        for i in range(n):
            print(i)
            F_kfac = F_kfac_list[i]
            F_kfac_org = F_kfac_list_org[i]
            ...
            for layer_id in F_kfac.data.keys():
                F_kfac.data[layer_id] = list(F_kfac.data[layer_id])
                dummy=50
                F_kfac.data[layer_id][0] = compress_mat(F_kfac_org.data[layer_id][0], dummy, dummy,False)
                F_kfac.data[layer_id][1] = compress_mat(F_kfac_org.data[layer_id][1], dummy, dummy,False)

            model_vector_quantized = quantize_layer(model_vectors[i], models[0], None)
            model_proxy = copy.deepcopy(models[0])
            vector_to_parameters(model_vector_quantized, model_proxy.parameters())
